...
import { Activity, FoodSpot, Movie } from '../types';
...
export const TabDiscovery: React.FC<Props> = ({ activities, foodSpots, movies }) => {
  ...
  const handleDecide = async () => {
    ...
    if (pool.length === 0) {
      alert("Nessun elemento disponibile con questi filtri!");
      setLoading(false);
      return;
    }
    ...
    if (mode === 'fate') {
      ...
      setResult({
        reason: "Le stelle si sono allineate.",
        matchScore: 100
      });
      setLoading(false);
      return;
    ...
    try {
      ...
      if (decision && decision.selectedId) {
        ...
        setResult({ reason: "Scelta di riserva dell'Oracolo.", matchScore: 50 });
      }
    } catch (e) {
      setChosenItem(pool[Math.floor(Math.random() * pool.length)]);
      setResult({ reason: "Scelta offline.", matchScore: 50 });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      <div className="text-center space-y-2">
        <h2 className="text-3xl font-bold text-slate-800 dark:text-white">Scoperta</h2>
        <p className="text-slate-500 dark:text-slate-400">Lascia scegliere al sistema.</p>
      </div>
      ...
      <div className="glass-card p-6 rounded-[2rem] max-w-xl mx-auto space-y-8 shadow-xl dark:bg-slate-800">
        ...
        <div className="flex p-1.5 bg-slate-100 dark:bg-slate-700 rounded-2xl">
          <button onClick={() => { setDomain('Activity'); setResult(null); }} className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${domain === 'Activity' ? 'bg-white dark:bg-slate-600 shadow-sm text-fuchsia-600 dark:text-fuchsia-300' : 'text-slate-400'}`}>Attività</button>
          <button onClick={() => { setDomain('Food'); setResult(null); }} className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${domain === 'Food' ? 'bg-white dark:bg-slate-600 shadow-sm text-orange-600 dark:text-orange-300' : 'text-slate-400'}`}>Cibo</button>
          <button onClick={() => { setDomain('Media'); setResult(null); }} className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${domain === 'Media' ? 'bg-white dark:bg-slate-600 shadow-sm text-rose-600 dark:text-rose-300' : 'text-slate-400'}`}>Media</button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <button onClick={() => setMode('fate')} className={`p-6 rounded-3xl border-2 flex flex-col items-center gap-3 transition-all ${mode === 'fate' ? getThemeColor() : 'border-slate-100 dark:border-slate-700 hover:border-slate-200 text-slate-400'}`}>
            ...
            <div className="text-center"><span className="block font-bold">Fato Cieco</span><span className="text-[10px] opacity-70">Scelta casuale</span></div>
          </button>
          <button onClick={() => setMode('smart')} className={`p-6 rounded-3xl border-2 flex flex-col items-center gap-3 transition-all ${mode === 'smart' ? getThemeColor() : 'border-slate-100 dark:border-slate-700 hover:border-slate-200 text-slate-400'}`}>
            ...
            <div className="text-center"><span className="block font-bold">Match Intelligente</span><span className="text-[10px] opacity-70">Analisi IA</span></div>
          </button>
        </div>
        ...
        {mode === 'smart' && (
          <div className="space-y-6 pt-6 border-t border-dashed border-slate-200 dark:border-slate-600 animate-slide-in">
            <h3 className="text-center text-xs font-bold text-slate-400 uppercase tracking-widest">Calibra Selezione</h3>
                ...
                <>
                  <div className="bg-white/50 dark:bg-slate-700/50 p-4 rounded-2xl">
                    <div className="flex justify-between items-center mb-2">
                      <div className="flex items-center gap-2 text-fuchsia-700 dark:text-fuchsia-300 font-bold text-sm"><Battery className="w-4 h-4" /> Energia</div>
                  ...
                  <div className="bg-white/50 dark:bg-slate-700/50 p-4 rounded-2xl">
                    <div className="flex justify-between items-center mb-2">
                      <div className="flex items-center gap-2 text-fuchsia-700 dark:text-fuchsia-300 font-bold text-sm"><Wallet className="w-4 h-4" /> Budget</div>
                  ...
                  <div className="bg-white/50 dark:bg-slate-700/50 p-4 rounded-2xl">
                     <div className="flex items-center gap-2 text-fuchsia-700 dark:text-fuchsia-300 font-bold text-sm mb-2"><MapPin className="w-4 h-4" /> Posizione</div>
              ...
              {domain === 'Food' && (
                <>
                  <div className="flex items-center gap-3 p-3 bg-orange-50 dark:bg-orange-900/30 rounded-xl border border-orange-100 dark:border-orange-900">
                    ...
                    <span className="text-sm font-bold text-orange-800 dark:text-orange-200">Includi posti visitati?</span>
                  </div>
                  <div className="bg-white/50 dark:bg-slate-700/50 p-4 rounded-2xl">
                     <div className="flex items-center gap-2 text-orange-700 dark:text-orange-300 font-bold text-sm mb-2"><Utensils className="w-4 h-4" /> Cucina</div>
              ...
              {domain === 'Media' && (
                 <>
                  <div className="flex items-center gap-3 p-3 bg-rose-50 dark:bg-rose-900/30 rounded-xl border border-rose-100 dark:border-rose-900">
                    ...
                    <span className="text-sm font-bold text-rose-800 dark:text-rose-200">Includi re-watch?</span>
                  </div>
                  <div className="bg-white/50 dark:bg-slate-700/50 p-4 rounded-2xl">
                     <div className="flex items-center gap-2 text-rose-700 dark:text-rose-300 font-bold text-sm mb-2"><Film className="w-4 h-4" /> Genere / Mood</div>
        ...
        >
          {loading ? (
            <>
              ...
              <span>Consultando gli Spiriti...</span>
            </>
          ) : (
            <>
              <Sparkles className="w-5 h-5" />
              <span>Fai la Scelta</span>
            </>
          )}
        </button>
      </div>
        ...
        <div className="max-w-xl mx-auto animate-scale-up">
           ...
           `}>
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">
                {mode === 'fate' ? 'La Ruota del Fato ha scelto' : "L'Oracolo Raccomanda"}
              </h2>
              ...
              <div className="bg-white/60 dark:bg-slate-700/60 p-6 rounded-2xl border border-white dark:border-slate-600">
                 ...
                 {mode === 'smart' && (
                   <div className="mt-4 flex justify-center items-center gap-2">
                      <span className="text-xs font-bold text-slate-500 dark:text-slate-400">{result.matchScore}% Affinità</span>
                   </div>
                 )}
              </div>
           </div>
